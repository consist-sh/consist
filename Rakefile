...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("consist.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in consist.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "consist.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
